...
pub struct Stats {
    /// Input tokens actually read, at any price (see `CompletionResponse`).
    pub tokens_in: u64,
    pub tokens_out: u64,
    /// The same input, weighted into base-input-price units — what
    /// `estimate_cost_usd` must be given once prompt caching is in play.
    pub billable_tokens_in: u64,
...
impl Stats {
    pub(crate) fn merge(&mut self, other: &Stats) {
        ...
        self.billable_tokens_in += other.billable_tokens_in;
...
pub struct RunProgress {
    pub phase: Phase,
    /// This chapter was publisher apparatus and went to no model. Reported so
    /// a skipped page never looks like a fully-cached one.
    pub apparatus: bool,
...
pub struct RunSummary {
    ...
    pub tokens_in: u64,
    pub tokens_out: u64,
    /// The same input weighted into base-input-price units; pass THIS to
    /// `estimate_cost_usd`, not `tokens_in`.
    pub billable_tokens_in: u64,
    pub units_translated: usize,
    pub units_failed: usize,
...
async fn run_inner(
    ...
    // Recorded so a later resume can say "8 of 12 chapters" without re-parsing
    // the book. Metadata only: it enters no cache key and re-bills nothing.
    store.set_meta("total_chapters", &total_ch.to_string())?;
    ...
    for ci in 0..total_ch {
        ...
        // Units this chapter could not get from the model. They are deliberately
        // NOT cached (see `store_chapter`), so the next run pays for them again.
        let mut chapter_failed = 0usize;
        ...
        if do_translate {
            ...
            total.merge(&stats);
            chapter_failed += stats.units_failed;
        ...
        // "done" is what a resume reads as "already paid for", so it has to mean
        // every unit of this chapter reached the cache. A chapter that kept
        // some source text because a call failed WILL be re-sent and re-billed
        // next run; recording it as done would let the confirm gate quote a
        // discount the run does not honour — worst case, a whole book that
        // failed against an unreachable endpoint would price as free.
        store.set_chapter_status(
            ci,
            &book.chapters[ci].href,
            if chapter_failed == 0 {
                "done"
            } else {
                "partial"
    ...
    let mut summary = RunSummary {
        ...
        billable_tokens_in: total.billable_tokens_in + anno_stats.billable_tokens_in,
    ...
    if let (Some(anno), Some(asig)) = (cfg.annotations.as_ref(), anno_sig.as_deref()) {
        if store.get_meta(&anno_reviewed_key(asig))?.is_none() {
            ...
            summary.billable_tokens_in += stats.billable_tokens_in;
...
async fn translate_once(
    ...
    stats.billable_tokens_in += resp.billable_tokens_in;
    ...
    match validate::parse_and_check(&resp.text, &units) {
        ...
        Err(crate::CoreError::Validation(why)) if why.contains("placeholder mismatch") => {
            ...
            stats.billable_tokens_in += resp2.billable_tokens_in;
