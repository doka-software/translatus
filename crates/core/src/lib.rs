...
///
/// `tokens_in` must be the PRICE-WEIGHTED input count
/// (`RunSummary::billable_tokens_in`), not the raw token total. Prompt-cache
/// reads bill at 0.1x and cache writes at 1.25x/2x, so a cached run's raw total
/// priced at the base rate reads far too high; the weighting is applied at the
/// provider boundary, where the per-bucket breakdown exists. Without caching the
...
/// 其價格 arm 保留只作歷史輸入的容錯）。
///
/// Introductory and promotional rates are deliberately NOT encoded: a
/// date-dependent price is a time bomb in a pure function, and erring high means
/// the estimate is a ceiling rather than a figure the user can be under-quoted
/// against. Claude Sonnet 5's $2/$10 is no longer one of them — the official
/// pricing page (checked 2026-09-01) records that the scheduled rise to $3/$15
/// was cancelled and $2/$10 is the standard rate, so it gets its own arm ahead
/// of the Sonnet 4.x families, which remain $3/$15.
///
/// Match order is specific→general: `gpt-5.4-mini`/`-nano` must precede the bare
/// `gpt-5.4`, `gpt-4o-mini` precede `gpt-4o`, and `claude-sonnet-5` precede the
/// bare `claude-sonnet`, or the broader arm wins first. Keep the Claude arms
/// grouped so a new tier is obvious to add.
pub fn estimate_cost_usd(model: &str, tokens_in: u64, tokens_out: u64) -> f64 {
    let (pin, pout): (f64, f64) = match model {
        ...
        // Specific before general: `claude-sonnet-5` must precede the bare
        // `claude-sonnet` arm or it silently prices 50% high.
...
mod cost_tests {
    ...
    fn every_offered_claude_model_is_priced_and_tiers_are_ordered() {
        ...
        for (model, expected) in [
            ...
            ("claude-sonnet-5", 12.0),   // 2 + 10 (standard since the rise was cancelled)
            ("claude-opus-5", 30.0),     // 5 + 25
