...
- `--mode replace|bilingual` — translation only, or original + translation
...
passes), so they are much larger than the visible note text. The command is
...
shelling out. Registration is one line for Claude Code:
...
- **Credentials.** For the provider's normal endpoint, the server uses the
  ...
  caller-supplied `base_url` receives no ambient key. To use an authenticated
  subscription sidecar through MCP, the operator launching the MCP server must
  bind its token to one exact URL:
...
- **Sidecar refuses connections** — is `npm start` still running? The kit
  binds to `127.0.0.1:8765` and only accepts loopback requests.
