...
<p align="center">開源 CLI，用你自己的 LLM，在本機翻譯、註解整本書。</p>
...
- **中斷不花一毛錢。** 每一段都存進本機 SQLite 快取。當機、暫停、下週再翻，
  ...
  零 LLM 呼叫。
- **註解，為你而寫。** 用勾的就能開始：九種服務（術語解釋到世界連結）、
  講解深淺（入門白話到內行）、書齋或口語聲線；也可以寫下你是誰，或乾脆
  讓你自己的 AI 替你填好整份讀者側寫（[schema 與標準 prompt](docs/READER-PROFILE.md)）。
  引擎先畫出全書線索圖，只挑值得停留的段落落註：段落之前鋪墊，或之後
  回指你讀過的內容（絕不預告後文），最後把所有註解統一校閱一次。註解
...
- **也為 agent 而生。** CLI 講結構化 JSON、冪等、可安全續跑；還內建 MCP
  server：`translatus mcp` 讓 Claude Code 等 agent 直接估價、翻譯、註解
  （[見下方](#接上你的-agentmcp)）。
...
macOS（Apple silicon 或 Intel）預編譯執行檔放進 `PATH`，或用 Rust（stable）
從原始碼安裝：
...
它會找出你附近的書，指令列做得到的事它都做得到：**翻譯**與**註解**是兩個
可獨立開關的服務，所以關掉翻譯、留著註解，就是「只加註解、不動原文」。每本書
可以各自設定目標語言、一般或專家深度、版面，以及註解的四件事：你是誰、為什麼
讀這本、想要哪幾種幫助、註解多密、用什麼語言寫。設定畫面則管模型來源（訂閱、
...
功能怪怪的？`translatus doctor` 一次檢查執行檔本身、訂閱 sidecar、sidecar
的埠，以及每個 agent 的 MCP 註冊狀態，並直接告訴你該修什麼。

移除 Translatus：

1. 移除執行檔：用 Homebrew 裝的執行 `brew uninstall translatus`；手動下載的
   直接刪掉你放在 `PATH` 上的那個檔案；用 `cargo install` 裝的執行
   `cargo uninstall translatus`。
2. 先移除 MCP 註冊（趁執行檔還在）：`translatus mcp uninstall`
   （或用 `--client claude`／`--client codex` 只移除單一 client）。
3. 刪除本機設定：`rm -rf "$HOME/Library/Application Support/translatus"`
   （或 `TRANSLATUS_CONFIG_DIR` 指到的位置）。
4. API 金鑰存在系統鑰匙圈，不在檔案裡：打開「鑰匙圈存取」，搜尋
   「translatus」，刪除找到的項目。

你的書與翻譯成品不會被動到，那些是你自己選位置存放的一般檔案。
...
安裝健康狀態（binary、sidecar、埠、各 client 註冊，見[解除安裝與診斷](#解除安裝與診斷)）。
...
- **[FAQ（英文）](docs/FAQ.md)**：成本、訂閱合規、引擎為何免費、資料去向、
  與先行專案的差異。
- **[註解調校地圖（英文）](docs/ANNOTATION-TUNING.md)**：給要改註解品質的貢獻者。
- **[CONTRIBUTING.md](CONTRIBUTING.md)** · **[SECURITY.md](SECURITY.md)** ·
  **[CHANGELOG.md](CHANGELOG.md)**

## 註解不做的事

註解遵守寫進引擎 prompt 的硬規則，不是靠自律：
...
- 原文在輸出中逐 byte 保留；譯文與註解是分離、有標示的區塊。
...
引擎與 CLI 採 MIT 授權、永遠免費，功能不會為了付費方案打折。如果 Translatus
幫你把一本書渡了過來，這是幾種讓它走下去的方式：

- **參與：** 給 [doka-software/translatus](https://github.com/doka-software/translatus)
  一顆星、回報問題，或送一個 PR，任何尺寸都算數。
- **贊助：** [Ko-fi](https://ko-fi.com/dokasoftware) <!-- SPONSOR-LINKS -->

維護者：[@doka-doka](https://github.com/doka-doka)。
