...
pub enum NoteVoice {
    ...
    #[default]
    Study,
    /// 口語：the spoken register — more conversational, a higher share of
    /// short reaction notes. Same hard rules, same length ceiling.
    ///
    /// The variant name is the on-disk and on-the-wire identifier (it enters
    /// `annotation_signature` as its `Debug` form), so it stays `Companion`
    /// whatever the interface calls the register.
    Companion,
}
...
impl std::str::FromStr for NoteVoice {
    type Err = String;
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        // The canonical values are `study` / `companion`; everything else is an
        // alias. The 陪讀 aliases predate the 口語 rename and stay accepted
        // forever: profiles already written to disk must keep parsing.
        match s.trim().to_lowercase().as_str() {
            "study" | "書齋" | "书斋" => Ok(NoteVoice::Study),
            "companion" | "spoken" | "口語" | "口语" | "陪讀" | "陪读" => {
                Ok(NoteVoice::Companion)
            }
...
mod tests {
    ...
    fn annotation_signature_is_sensitive_and_independent() {
        ...
        // GOLDEN, same contract as `cache_signature`: this value is on disk in
        // every `.etjob`. Prompt WORDING is deliberately outside the hash (only
        // the salt stands in for it), so editing a prompt sentence — a
        // punctuation instruction, a voice label — must leave this untouched
        // and keep every cached note valid. Moving it re-annotates every book
        // in the world; change it only with the salt, deliberately.
        assert_eq!(
            sig, "515ae40e462687ad59e6a756e9113882a93cb3830ba0c3fc49d614b2c19d12dc",
            "annotation_signature drifted — this re-annotates every cached book; intentional?"
        );
        // Deterministic.
    ...
    /// The label the interface prints moved from 陪讀 to 口語 (2026-09-01), so
    /// both spellings must reach the same register: a profile written under the
    /// old label keeps working, and a user reading the new one can type it.
    /// The canonical value, and everything that reaches the cache, is unchanged.
    #[test]
    fn every_voice_alias_lands_on_the_same_register() {
        for s in ["companion", "spoken", "口語", "口语", "陪讀", "陪读"] {
            assert_eq!(
                s.parse::<NoteVoice>().unwrap(),
                NoteVoice::Companion,
                "`{s}` must parse as the spoken register"
            );
        }
        for s in ["study", "書齋", "书斋"] {
            assert_eq!(s.parse::<NoteVoice>().unwrap(), NoteVoice::Study);
        }
        assert!("chatty".parse::<NoteVoice>().is_err());
        // The wire form is the identifier, never the label.
        assert_eq!(
            serde_json::to_string(&NoteVoice::Companion).unwrap(),
            "\"companion\""
        );
        // …and renaming the label cannot move the cache key.
        let mut c = anno_cfg();
        c.annotations.as_mut().unwrap().voice = "口語".parse().unwrap();
        let mut d = anno_cfg();
        d.annotations.as_mut().unwrap().voice = "陪讀".parse().unwrap();
        assert_eq!(
            c.annotation_signature().unwrap(),
            d.annotation_signature().unwrap()
        );
    }
