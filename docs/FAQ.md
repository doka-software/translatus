...
One command covers the clients you have:

```bash
translatus mcp install
```

It goes through each client's own CLI, and the first interactive run offers to
do it for you — a package install never edits another program's config by
itself. `translatus mcp uninstall` reverses it.
