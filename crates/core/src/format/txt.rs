...
pub fn write(doc: &TxtDoc, book: &Book, out: &Path, mode: OutputMode) -> Result<()> {
    ...
    if let Some(chapter) = book.chapters.first() {
        for seg in &chapter.segments {
            if let Some(t) = &seg.target {
                ...
                let restored = crate::format::placeholder::restore(t, &seg.placeholders);
...
mod tests {
    ...
    fn write_restores_glossary_sentinels() {
        ...
        write(&doc, &book, &out, OutputMode::Replace).unwrap();
