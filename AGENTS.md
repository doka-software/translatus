...
- Or register the MCP server: `translatus mcp install` (registers with the
  ...
  carry every parameter. `mcp uninstall` removes only what `mcp install`
  created; `--client claude|codex` scopes either verb to one client.
- Stuck or unsure of the install's state? `translatus doctor` (add `--json`)
  checks binary/PATH consistency, the sidecar kit + Node, the default sidecar
  port, per-client MCP registration, and the settings file.
- Over MCP, `base_url` only carries the user's stored token when it exactly
  matches the endpoint saved in Settings — any other URL gets no ambient
  credentials (the schema description states this rule).
