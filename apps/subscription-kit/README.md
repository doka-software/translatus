...
# 埠被占用？換一個：LLM_SUB_KIT_PORT=8790 npm start
...
### `/v1/chat/completions` 的 `usage`

輸入 token 分三種價位回報，因為它們的單價本來就不同（官方 prompt caching 計價，
2026-09-01 查證：cache read 0.1x、5 分鐘 cache write 1.25x、1 小時 cache write 2x
基礎 input 價；來源 <https://platform.claude.com/docs/en/about-claude/pricing#prompt-caching>）。
...
- `prompt_tokens` 是**這次請求真正讀進去的全部 input token**（含快取），與 OpenAI 語意一致。
  不要拿 Anthropic 原始的 `usage.input_tokens` 當總量：開了 prompt caching 之後，
  那個欄位只剩最後一個 cache breakpoint 之後的未快取尾巴，實測 16,183 token 的請求會回報 10。
- `cached_tokens` 是 OpenAI 既有欄位（快取命中，0.1x）。`cache_creation_*` 是本 kit 的擴充，
  因為 OpenAI 沒有 cache write 這個計費概念，而 Anthropic 有，且 TTL 決定 1.25x 還是 2x。
- **不要把三種加總後套基礎 input 單價**：那會把嚴重低估翻成嚴重高估。要算錢就分別加權。
- Codex 路徑同一個形狀：Codex 的 `input_tokens` 本來就含 `cached_input_tokens`，
  kit 會把它拆成未快取尾巴 + `cached_tokens`，總量不會被重複計算。
