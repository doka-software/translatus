...
pub(crate) fn extract_with_limits(path: &Path, limits: &Limits) -> Result<(Book, EpubDoc)> {
    ...
    // What the book itself declares as publisher apparatus. Nav documents are
    // read from the entries already in memory, so this costs no extra I/O.
    let nav_docs: Vec<(String, String)> = entries
        .iter()
        .filter(|(n, _)| n.ends_with(".xhtml") || n.ends_with(".html"))
        .map(|(n, b)| (n.clone(), String::from_utf8_lossy(b).to_string()))
        .filter(|(_, doc)| doc.contains("landmarks"))
        .collect();
    let apparatus_hrefs = crate::format::frontmatter::declared(&opf, &nav_docs);
    ...
    for idref in &spine {
        ...
        let plain: String = segments
            ...
            .collect::<Vec<_>>()
            .join(" ");
        let apparatus = apparatus_hrefs.contains(&href_raw)
            || crate::format::frontmatter::looks_like_apparatus(&plain, segments.len());
...
/// The default rules deliberately set NO text colour: the reader's theme owns
/// it, so the translation stays legible on light and dark grounds alike (a
/// hardcoded near-black used to vanish into night-mode backgrounds). Only the
/// accent border is tuned per scheme.
const BILINGUAL_CSS: &str = "<style type=\"text/css\">.etc-trans{margin:.15em 0 .9em;padding-left:.6em;border-left:2px solid #5B5BD6;}@media (prefers-color-scheme: dark){.etc-trans{border-left-color:#8F8FE8;}}</style>";

/// Vertical-writing variant. In vertical-rl (tategumi) and rtl-progression
/// books the block axis runs right-to-left, so a physical LEFT accent sits on
/// the far side of the translation — visually attached to the NEXT paragraph.
/// The accent, padding, and inter-block margins mirror to the right edge,
/// which is the reading-start side there (and equally correct for horizontal
/// rtl scripts). Colour rules are identical to the horizontal sheets.
const BILINGUAL_CSS_VERTICAL: &str = "<style type=\"text/css\">.etc-trans{margin:0 .15em 0 .9em;padding-right:.6em;border-right:2px solid #5B5BD6;}@media (prefers-color-scheme: dark){.etc-trans{border-right-color:#8F8FE8;}}</style>";
...
/// Like the translation, the default rules inherit the reader theme's text
/// colour (readable everywhere, including readers that ignore media queries);
/// the vermilion ink is layered per scheme — deep on paper, pale on night.
/// E-ink notes: monochrome readers collapse colour to luminance, their "dark
/// mode" is usually inversion rather than a colour scheme, and a low-alpha
/// tint either vanishes or dithers on a 16-level greyscale panel. The
/// `(monochrome)` layer (last, so it wins over the scheme layers) drops the
/// tint and returns the ink to the device's own text colour; the border and
/// the note prefix carry the distinction. Every accent keeps ≥5:1 luminance
/// contrast against its ground, so colour panels (Kaleido) that desaturate
/// stay legible too.
const NOTE_CSS: &str = "<style type=\"text/css\">.etc-note{background:rgba(178,58,42,.055);margin:.15em 0 .9em;padding:.35em .6em;border-left:2px solid #b23a2a;font-size:.92em;}@media (prefers-color-scheme: light){.etc-note{color:#5c2d24;}}@media (prefers-color-scheme: dark){.etc-note{color:#EAC0B5;background:rgba(230,120,100,.12);border-left-color:#CF6A55;}}@media (monochrome){.etc-note{background:none;color:inherit;}}</style>";

/// See [`BILINGUAL_CSS_VERTICAL`] — same mirroring for note blocks.
const NOTE_CSS_VERTICAL: &str = "<style type=\"text/css\">.etc-note{background:rgba(178,58,42,.055);margin:0 .15em 0 .9em;padding:.6em .35em;border-right:2px solid #b23a2a;font-size:.92em;}@media (prefers-color-scheme: light){.etc-note{color:#5c2d24;}}@media (prefers-color-scheme: dark){.etc-note{color:#EAC0B5;background:rgba(230,120,100,.12);border-right-color:#CF6A55;}}@media (monochrome){.etc-note{background:none;color:inherit;}}</style>";

/// Does the book read vertically (or right-to-left)? True when any stylesheet
/// or document declares `vertical-rl`, or the OPF spine declares
/// `page-progression-direction="rtl"`. Decided once per book: a vertical novel
/// is vertical throughout, and a right-edge accent on stray horizontal front
/// matter is still legible, while the reverse (left accent in a vertical body)
/// visually attaches the marker to the wrong paragraph.
pub(crate) fn is_vertical(entries: &[(String, Vec<u8>)]) -> bool {
    entries.iter().any(|(name, bytes)| {
        let lower = name.to_ascii_lowercase();
        let relevant = lower.ends_with(".css")
        ...
        s.contains("vertical-rl")
            || (lower.ends_with(".opf") && s.contains("page-progression-direction=\"rtl\""))
    })
}
...
pub fn write(
    ...
    let (bilingual_css, note_css) = if is_vertical(&doc.entries) {
        (BILINGUAL_CSS_VERTICAL, NOTE_CSS_VERTICAL)
    } else {
        (BILINGUAL_CSS, NOTE_CSS)
    };
    ...
    for (href, dom) in &doc.docs {
        ...
        let mut noted = false;
        // Apparatus is emitted exactly as it came in, whatever the cache holds.
        // Skipping only the model call was not enough: a job cache written
        // before this rule existed still carried a translation for the
        // copyright page, and the writer happily used it.
        if let Some(chapter) = chapter_index.get(href.as_str()).filter(|c| !c.apparatus) {
            ...
            for seg in &chapter.segments {
                ...
                if let Some(n) = seg.note.as_ref().filter(|n| !n.is_skip()) {
                    notes.insert(
                        seg.block_index,
                        crate::document::Note::new(
                            n.pos,
                            // Notes are prose in the reader's language too, and
                            // they name people constantly, so they need the same
                            // interpunct normalisation the translation gets.
                            crate::format::typography::normalize(
                                &escape_min(&n.text),
                                note_lang.as_deref().unwrap_or(&lang),
                            ),
                        ),
                    );
                }
...
mod tests {
    ...
    // Dark-mode regression: a hardcoded dark text colour in the injected CSS
    // made translations and notes unreadable on night themes (dark ground +
    // author-forced near-black ink). The default
    // rules must never set a text colour — the reader theme owns it — and any
    // scheme-specific ink may only live behind a prefers-color-scheme guard.
    #[test]
    fn injected_css_inherits_reader_theme_colour() {
        for css in [
            BILINGUAL_CSS,
            NOTE_CSS,
            BILINGUAL_CSS_VERTICAL,
            NOTE_CSS_VERTICAL,
        ] {
            let default_rules = css.split("@media").next().unwrap();
            assert!(
                !default_rules.contains("color:#"),
                "default rules must not hardcode a text colour: {default_rules}"
            );
            assert!(
                css.contains("prefers-color-scheme: dark"),
                "each injected sheet must carry a dark-scheme override: {css}"
            );
        }
        // E-ink regression: on monochrome panels the tint dithers or vanishes
        // and schemes are inversion-based, so the note must fall back to the
        // device's own ink. The guard must come last so it wins both schemes.
        for css in [NOTE_CSS, NOTE_CSS_VERTICAL] {
            let mono = css
                .split("@media (monochrome)")
                .nth(1)
                .expect("every note sheet must carry a monochrome layer");
            assert!(mono.contains("background:none") && mono.contains("color:inherit"));
        }
    }

    // Vertical regression: in vertical-rl the block axis runs right-to-left,
    // so a left accent visually attaches to the NEXT paragraph. Vertical (and
    // rtl-progression) books must get the mirrored sheets, and those sheets
    // must never carry a left-edge accent.
    #[test]
    fn vertical_books_get_right_edge_accents() {
        for css in [BILINGUAL_CSS_VERTICAL, NOTE_CSS_VERTICAL] {
            assert!(css.contains("border-right"), "vertical sheet: {css}");
            assert!(!css.contains("border-left"), "vertical sheet: {css}");
            assert!(!css.contains("padding-left"), "vertical sheet: {css}");
        }
        let horiz: Vec<(String, Vec<u8>)> = vec![
            ("OEBPS/s.css".into(), b"p{margin:0}".to_vec()),
            ("OEBPS/c.opf".into(), b"<spine>".to_vec()),
        ];
        assert!(!is_vertical(&horiz));
        let tategumi: Vec<(String, Vec<u8>)> = vec![(
            "OEBPS/s.css".into(),
            b"html{writing-mode: vertical-rl;}".to_vec(),
        )];
        assert!(is_vertical(&tategumi));
        let rtl_spine: Vec<(String, Vec<u8>)> = vec![(
            "OEBPS/c.opf".into(),
            br#"<spine page-progression-direction="rtl">"#.to_vec(),
        )];
        assert!(is_vertical(&rtl_spine));
        // A css-only rtl marker in a non-opf file must not trigger by itself.
        let stray: Vec<(String, Vec<u8>)> = vec![(
            "OEBPS/t.xhtml".into(),
            br#"<p>page-progression-direction="rtl"</p>"#.to_vec(),
        )];
        assert!(!is_vertical(&stray));
    }
    ...
    /// A cache written before the apparatus rule existed still holds a
    /// translation for the copyright page. Skipping only the model call let
    /// the writer use it anyway, which is how the page came back translated.
    #[test]
    fn apparatus_is_written_unchanged_even_when_the_cache_has_a_translation() {
        let path = write_zip("tx-apparatus-write.epub", &epub_entries(&[]));
        let (mut book, doc) = extract(&path).unwrap();
        book.chapters[0].apparatus = true;
        book.chapters[0].segments[0].target = Some("這頁不該被翻".into());

        let out = std::env::temp_dir().join("tx-apparatus-out.epub");
        write(&doc, &book, &out, OutputMode::Replace, "繁體中文", None).unwrap();

        let written = zip::ZipArchive::new(std::fs::File::open(&out).unwrap())
            .and_then(|mut z| {
                use std::io::Read;
                let mut s = String::new();
                z.by_name("OEBPS/ch1.xhtml")?.read_to_string(&mut s)?;
                Ok(s)
            })
            .unwrap();
        assert!(
            !written.contains("這頁不該被翻"),
            "apparatus must ignore a cached translation: {written}"
        );
        assert!(written.contains("Hello."), "the original must survive");
    }
