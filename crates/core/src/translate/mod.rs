...
pub type PairSink<'a> = &'a mut (dyn FnMut(Vec<(String, String)>) + Send);
...
async fn run_inner(
    ...
    for ci in 0..total_ch {
        ...
        if do_translate {
            ...
            total.merge(&stats);
            store.store_chapter(&book.chapters[ci], sig)?;
...
async fn translate_once(
    ...
    validate::parse_and_check(&resp.text, &units)
}
...
mod tests {
    ...
    async fn run_units(concurrency: usize) -> (HashMap<u64, String>, usize, Stats) {
        ...
        let mut sink = |pairs: Vec<(String, String)>| streamed += pairs.len();
        let map =
