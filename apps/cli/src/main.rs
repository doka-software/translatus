...
enum Command {
    ...
    /// Register with: `claude mcp add translatus -- translatus mcp`
    Mcp,
}
...
fn estimate_numbers(
    ...
    let tokens_in = book.est_source_tokens() as u64;
    // crude: output ≈ 1.1× input for CJK targets
    let tokens_out = (tokens_in as f64 * 1.1) as u64;
...
fn run_estimate(a: EstimateArgs, json_out: bool) -> Result<()> {
    ...
    let est = estimate_numbers(&book, &a.model, density);
    if !json_out {
        println!(
            "{} → {} ({}): {}, {}",
            human_path(&a.input),
...
async fn run_translate(a: TranslateArgs, json_out: bool, ui: Ui) -> Result<RunOutcome> {
                ...
                .map(|(i, ch)| {
                    (
                        ch.title
                            ...
                            .unwrap_or_else(|| tui::i18n::tr1("run.chapter.title", &(i + 1).to_string())),
    ...
    if !json_out && ui == Ui::Cli {
        ...
        if !matches!(provider_kind, ProviderKind::Mock) {
            ...
            print_run_estimate(&estimate_numbers(&book, &cfg.model, density), &cfg.model);
        }
    }
    ...
    let summary = translate::run(&provider, &cfg, &mut book, &store, &sig, |p| {
        ...
        if let Some(b) = board.as_mut() {
            if p.phase == translate::Phase::Translating {
    ...
    if let Some(b) = board.as_ref() {
        ...
        println!("{}", tui::i18n::tr("run.resume"));
        return Ok(RunOutcome { units_failed: 0 });
    }
...
async fn run_annotate(a: AnnotateArgs, json_out: bool) -> Result<RunOutcome> {
    ...
    if !json_out {
        ...
        if !matches!(provider_kind, ProviderKind::Mock) {
            let density = cfg.annotations.as_ref().map(|an| an.density);
            if let Some((ai, ao, ac)) = estimate_numbers(&book, &cfg.model, density).annotation {
                if ac > 0.0 {
                    eprintln!(
