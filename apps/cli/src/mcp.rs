...
fn estimate_params() -> Vec<P> {
    vec![
        ...
        P(
            ...
            Kind::Str,
            false,
            "Model id to price against (default gpt-5.4-mini).",
        ),
        P(
            "annotate",
            "--annotate",
...
async fn handle_tool_call(
    ...
    if arguments.get("base_url").is_some() {
        ...
        if let Some(requested) = arguments.get("base_url").and_then(Value::as_str) {
            let configured_url = std::env::var("TRANSLATUS_MCP_ENDPOINT_URL").ok();
            let configured_token = std::env::var("TRANSLATUS_MCP_ENDPOINT_TOKEN").ok();
            if let Some(token) = scoped_endpoint_token_for(
                requested,
