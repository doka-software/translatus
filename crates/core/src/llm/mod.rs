...
pub struct CompletionResponse {
    pub text: String,
    /// Every input token the request actually read, at any price — uncached
    /// tokens plus prompt-cache reads plus prompt-cache writes. This is the
    /// number to SHOW a user; it is deliberately NOT the number to multiply by
    /// the base input price.
    pub tokens_in: u64,
    pub tokens_out: u64,
    /// `tokens_in` re-expressed in base-input-price units, because the cache
    /// buckets are not billed at the base rate. Anthropic's published
    /// multipliers (verified 2026-09-01 against
    /// <https://platform.claude.com/docs/en/about-claude/pricing#prompt-caching>)
    /// are 0.1x for a cache read, 1.25x for a 5-minute cache write and 2x for a
    /// 1-hour cache write; OpenAI discounts cached input by the same 0.1x and
    /// charges nothing for writes.
    ///
    /// The weighting happens at the provider boundary, the only layer that sees
    /// the breakdown; everything above just needs one number to hand
    /// `estimate_cost_usd`. Equal to `tokens_in` when the provider reports no
    /// cache activity, so a plain gateway or Ollama is unaffected.
...
pub(crate) fn is_busy(e: &CoreError) -> bool {
    let m = e.to_string();
    // Match the status line or the kit's own error type, not a bare "429":
    // that substring turns up in token counts and request ids, and a false
    // positive here costs minutes of deliberate waiting.
    m.contains("429 Too Many Requests") || m.contains("llm_subscription_kit_busy")
}
...
mod busy_tests {
    ...
    fn a_busy_endpoint_is_not_treated_as_a_flaky_call() {
        ...
        for other in [
            ...
            // A bare "429" inside unrelated numbers must not read as busy.
            CoreError::Other("provider error: 500 (request 8f429ac, 429 tokens)".into()),
        ] {
