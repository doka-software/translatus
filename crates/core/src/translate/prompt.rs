...
6. 嚴格使用「{lang}」的標準字形、用語與標點：目標為繁體中文時一律繁體字（禁止混入簡體字）、台灣慣用語；目標為简体中文時一律簡體字。整本書字形必須一致。
7. 標點依目標語言的書寫系統，不可沿用原文的半形習慣：中文（繁簡皆同）用全形「，。：；！？（）」；日本語用全角「、。：；！？（）」；韓文與西方語言維持該語言原本的半形標點。譯文中若保留一段原文引句、程式碼、URL 或數字（1,234、3.14），該片段內部的標點維持原樣。
8. 同一專有名詞（人名、地名）全書譯法一致：若決定音譯就全程同一音譯，不得時而音譯時而保留原文。"#,
        lang = lang_display
    )
}
