...
OpenAI-compatible endpoint, keychain or env) / ollama (local) / subscription \
(a Codex/ChatGPT or Claude Code login: the server manages its own sidecar — no \
key, port, or base_url needed; prefer this over hand-running the sidecar). \
...
/// The known client vocabulary, for surfaces (doctor) that report per client.
pub fn known_clients() -> Vec<(&'static str, &'static str)> {
    CLIENTS.iter().map(|(bin, name, _)| (*bin, *name)).collect()
}

/// Which registrations this tool created (see [`provenance_file`]).
pub fn registered_by_us() -> Vec<String> {
    provenance_read()
}

/// Registrations THIS tool created, one client binary name per line. Written
/// on a successful `add`, removed on a successful `remove`. This is what lets
/// `uninstall` mean "undo what install did" instead of "remove every
/// registration it can find" — a registration the user made by hand (or that
/// another tool made) is not ours to delete without an explicit `--client`.
fn provenance_file() -> Option<std::path::PathBuf> {
    crate::tui::store::config_dir().map(|d| d.join("mcp-registered-by-us"))
}

fn provenance_read() -> Vec<String> {
    provenance_file()
        .map(|p| provenance_read_from(&p))
        .unwrap_or_default()
}

fn provenance_read_from(p: &std::path::Path) -> Vec<String> {
...
fn provenance_write(bins: &[String]) {
    if let Some(p) = provenance_file() {
        provenance_write_to(&p, bins);
    }
}

fn provenance_write_to(p: &std::path::Path, bins: &[String]) {
    if bins.is_empty() {
        let _ = std::fs::remove_file(p);
    } else if let Some(dir) = p.parent() {
        let _ = std::fs::create_dir_all(dir);
        let _ = std::fs::write(p, bins.join("\n"));
    }
}

/// Resolve a `--client` value to the known client table, or explain the
/// vocabulary. Accepts the binary name ("claude", "codex").
fn resolve_client(
    only: &str,
) -> Result<&'static (&'static str, &'static str, &'static [&'static str])> {
    CLIENTS
        .iter()
        .find(|(bin, _, _)| *bin == only)
        .ok_or_else(|| {
            let known: Vec<&str> = CLIENTS.iter().map(|(b, _, _)| *b).collect();
            anyhow::anyhow!(
                "unknown client '{only}' — known clients: {}",
                known.join(", ")
            )
        })
}
...
///
/// `only` narrows the operation to a single client. Without it, `install`
/// targets every client found; `uninstall` targets only the registrations this
/// tool recorded creating (see [`provenance_file`]) and points at `--client`
/// for anything else.
pub fn register(install: bool, only: Option<&str>) -> Result<Vec<String>> {
    if let Some(c) = only {
        resolve_client(c)?;
    }
    ...
    for (bin, name, extra) in CLIENTS {
        ...
        if !client_installed(bin) {
            if only.is_some() {
                out.push(format!(
                    "  {} {name}: `{bin}` not found on PATH",
                    super::tui::theme::FAIL
                ));
            }
            continue;
        }
        // Bare uninstall never touches a registration we did not create; the
        // client stays listed with a hint instead of silently losing state
        // that belonged to the user or to another installer.
        if !install && only.is_none() && !ours.iter().any(|b| b == bin) {
            out.push(format!(
                "  {} {name}: skipped — no registration here was added by \
                 `translatus mcp install`; if one exists and you want it gone, \
                 pass `--client {bin}`",
                super::tui::theme::REVIEW
            ));
            continue;
        }
        found += 1;
            ...
            Ok(o) if o.status.success() => {
                if install {
                    if !ours.iter().any(|b| b == bin) {
                        ours.push((*bin).to_string());
                    }
                } else {
                    ours.retain(|b| b != bin);
                }
                provenance_write(&ours);
                out.push(format!(
                    "  {} {name}: {}",
                    super::tui::theme::OK,
                    if install {
...
fn translate_params() -> Vec<P> {
    vec![
        ...
        P("provider", "--provider", Kind::Str, false, "openai | ollama | mock. Omit to use the model source saved in the user settings; pass mock only when a free dry run is what you want."),
        P("model", "--model", Kind::Str, false, "Model id for the chosen provider."),
        P("base_url", "--base-url", Kind::Str, false, "OpenAI-compatible endpoint override (local servers, subscription sidecar). CREDENTIAL RULE: the user's stored token is attached ONLY when this value exactly matches the endpoint saved in the user's Settings (or the TRANSLATUS_MCP_ENDPOINT_URL override); any other URL — including the same sidecar on a different port — gets NO ambient credentials and calls will be rejected by token-gated endpoints. If you must use a different endpoint, omit base_url and use the saved source, or use the managed subscription provider."),
        P("prompt", "--prompt", Kind::Str, false, "Tone / wording / audience instructions for the translation style."),
        P("mode", "--mode", Kind::Str, false, "bilingual (default) or replace."),
...
/// Fill in the model source from the user's saved settings when the caller did
/// not name one, and refuse outright when there is none to fill in.
///
/// The flag interface defaults `--provider` to `mock`, which is right there: it
/// is a dry run you ask for. Over MCP it was catastrophic. An agent told to
/// translate a book has no reason to pass a provider — the parameter is
/// optional and its description called mock "the default" — so the run went to
/// the mock provider, finished in a minute with no model call, wrote a 962 KB
/// EPUB containing 509 placeholder translations, and returned
/// `status: completed`. The agent then told its user the book was translated.
/// A wrong book reported as a right one is worse than any error.
fn apply_configured_model_source(
    arguments: &Value,
    argv: &mut Vec<String>,
) -> Result<(), ToolError> {
    if arguments.get("provider").and_then(Value::as_str).is_some() {
        return Ok(()); // the caller made an explicit choice, including "mock"
    }
    if arguments
        .get("cache_only")
        ...
        .unwrap_or(false)
    {
        return Ok(()); // re-render from cache makes no model calls at all
    }
    let api = crate::tui::store::load().api;
    let provider = api.provider.trim().to_string();
    if provider.is_empty() || provider == "mock" {
        // Nothing configured: use the sidecar this binary ships with rather
        // than refusing. It needs no key, no keychain and no setup — which is
        // the whole reason an agent can now be handed a book and just work.
        argv.push("--provider".into());
        argv.push("subscription".into());
        return Ok(());
    }
    argv.push("--provider".into());
    argv.push(provider);
    if !api.model.trim().is_empty() {
        ...
        argv.push(api.model.clone());
    }
    if let Some(url) = api.base_url.as_deref().filter(|u| !u.trim().is_empty()) {
        // Only the user's own saved endpoint reaches this path; a caller-named
        // one still goes through the stripped-credential branch.
        et_core::validate_base_url(url, et_core::EndpointTrust::CallerSupplied)
            .map_err(|e| ToolError::InvalidParams(format!("saved base_url is unusable: {e}")))?;
...
async fn handle_tool_call(
    ...
    let mut argv = build_argv(cmd, &spec, arguments)?;
    if cmd != "estimate" {
        apply_configured_model_source(arguments, &mut argv)?;
    }
...
mod tests {
    ...
    /// An agent that does not name a provider must never get a dry run. The
    /// flag interface defaults to `mock`, which over MCP produced a finished
    /// book of placeholders reported as `status: completed`.
    #[test]
    fn an_unspecified_provider_never_silently_becomes_mock() {
        let mut argv = vec!["--json".into(), "translate".into()];
        let err = apply_configured_model_source(&json!({"input": "b.epub"}), &mut argv);
        // With no configuration this must refuse. With configuration it must
        // fill in — either way, `mock` is never the answer.
        match err {
            Err(ToolError::InvalidParams(m)) => {
                assert!(m.contains("no model source"), "unhelpful message: {m}");
                assert!(!argv.iter().any(|a| a == "mock"));
            }
            Ok(()) => {
                let i = argv.iter().position(|a| a == "--provider");
                assert!(i.is_some(), "a provider must have been filled in: {argv:?}");
                assert_ne!(
                    argv[i.unwrap() + 1],
                    "mock",
                    "settings must not select mock"
                );
            }
    ...
    /// An explicit choice is still the caller's to make, including a dry run.
    #[test]
    fn an_explicit_provider_is_left_alone() {
        let mut argv = vec![
            "--json".into(),
            "translate".into(),
            "--provider".into(),
        ...
        apply_configured_model_source(&json!({"provider": "mock"}), &mut argv).unwrap();
        assert_eq!(argv, before);
    }

    /// Re-rendering from cache makes no model call, so it needs no source.
    #[test]
    fn cache_only_needs_no_model_source() {
        let mut argv = vec!["--json".into(), "translate".into()];
        apply_configured_model_source(&json!({"cache_only": true}), &mut argv).unwrap();
        assert!(!argv.iter().any(|a| a == "--provider"));
    }
    ...
    // Uninstall scope (real-world incident: a bare `mcp uninstall` removed a
    // registration the user had made long before, because "found" and "ours"
    // were the same set). The provenance record is what separates them; it
    // must round-trip, tolerate blank lines, and vanish when emptied.
    #[test]
    fn mcp_uninstall_scope_provenance_roundtrip() {
        let dir = std::env::temp_dir().join("et-mcp-provenance-test");
        let _ = std::fs::remove_dir_all(&dir);
        let p = dir.join("mcp-registered-by-us");
        assert!(provenance_read_from(&p).is_empty(), "missing file = empty");
        provenance_write_to(&p, &["claude".into()]);
        assert_eq!(provenance_read_from(&p), vec!["claude".to_string()]);
        provenance_write_to(&p, &["claude".into(), "codex".into()]);
        assert_eq!(provenance_read_from(&p).len(), 2);
        provenance_write_to(&p, &[]);
        assert!(!p.exists(), "an emptied record must not linger as a file");
        let _ = std::fs::remove_dir_all(&dir);
    }

    // `--client` takes only the known vocabulary, and says what it is.
    #[test]
    fn mcp_client_flag_rejects_unknown_names() {
        assert!(resolve_client("claude").is_ok());
        assert!(resolve_client("codex").is_ok());
        let err = resolve_client("cursor").unwrap_err().to_string();
        assert!(err.contains("claude") && err.contains("codex"), "{err}");
        assert!(register(false, Some("cursor")).is_err());
    }
}
