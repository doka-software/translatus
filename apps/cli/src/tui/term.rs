...
/// Drop raw mode while keeping the alternate screen.
///
/// Raw mode exists to read keys; the progress board only writes. Holding it
/// through a run means `^C` arrives as a keystroke nobody is listening for, so
/// the board's own "^C stop" legend was a lie for the whole length of the run.
/// Outside raw mode the terminal turns it back into SIGINT, which the process
/// already handles.
pub fn keys_off() {
    let _ = terminal::disable_raw_mode();
}

/// Take raw mode back after a run, so the menus can read keys again.
pub fn keys_on() {
