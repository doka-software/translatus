...
struct Row {
    ...
    /// This chapter came back from the job cache rather than from the model.
    /// Kept for the life of the row: the run still walks it and "finishes" it
    /// in milliseconds, and without the tag that reads as an impossibly fast
    /// translation instead of as work that was already paid for.
    restored: bool,
}
...
pub struct Board {
    ...
    prep: Option<String>,
    /// Expert level only checkpoints at chapter boundaries (mid-chapter work is
    /// un-reflected draft), so "progress is saved" is true per chapter, not per
    /// batch. Saying so is the difference between a promise and a surprise.
    chapter_granular: bool,
    started: Instant,
    frame: usize,
...
impl Board {
    ...
    /// Mark a chapter as already present in the job cache.
    ///
    /// Called before the run starts, so the board opens showing how much of the
    /// book is already done instead of a table of pending rows that then tick
    /// over faster than anyone can read. The row is Done from the first frame,
    /// counts towards the progress bar, and is excluded from the ETA — its
    /// near-zero "duration" is not throughput evidence.
    pub fn mark_restored(&mut self, idx: usize) {
        if let Some(r) = self.rows.get_mut(idx) {
            r.restored = true;
            r.state = State::Done;
        }
    ...
    /// Mark this run as checkpointing only at chapter boundaries.
    pub fn chapter_granular(&mut self) {
        self.chapter_granular = true;
    }
    ...
    fn remaining(&self) -> Option<Duration> {
        ...
        for r in self.rows.iter().filter(|r| !r.restored) {
            if let Some(t) = r.took {
                done_chars += r.chars;
                done_secs += t.as_secs_f64();
            }
        }
        // Restored chapters are neither measured throughput (they took no time)
        // nor remaining work (they are already done). Counting them either way
        // made the first ETA a fantasy.
        let left_chars: usize = self
            .rows
            .iter()
            .filter(|r| !r.restored && r.took.is_none())
            .map(|r| r.chars)
            .sum();
    ...
    fn paint(&mut self, force: bool) -> Result<()> {
        ...
        for r in self.rows.iter().skip(top).take(page) {
            ...
            let meta = if r.restored {
                format!("{:>7}  {}", fmt_count(r.chars), tr("run.cached"))
            } else {
                match (r.state, r.took) {
        ...
        f.blank();
        f.line(&theme::gray(if self.chapter_granular {
            tr("run.stop.expert")
        } else {
            tr("run.stop")
        }));
...
mod tests {
    ...
    /// A resumed run walks its cached chapters in milliseconds. Two things
    /// must not happen: the rows must not read as an impossibly fast
    /// translation, and their non-existent duration must not become the
    /// throughput the ETA extrapolates from.
    #[test]
    fn a_restored_chapter_is_done_on_arrival_and_never_feeds_the_eta() {
        let chapters = vec![
            ("Chapter 1".to_string(), 21_900usize),
            ("Chapter 2".to_string(), 10_200),
            ("Chapter 3".to_string(), 10_000),
        ];
        let mut b = Board::new("book.epub", chapters);
        b.interactive = false;

        b.mark_restored(0);
        assert!(b.rows[0].restored, "the row has to be able to say so");
        assert_eq!(
            b.rows[0].state,
            State::Done,
            "restored work is finished work"
        );
        assert_eq!(b.done_count(), 1, "the progress bar opens at 1/3, not 0/3");
        assert!(
            b.remaining().is_none(),
            "a chapter that took no time is not throughput evidence"
        );

        // One real chapter: 10,200 chars in 600s, 10,000 chars left. The
        // restored 21,900-char chapter must appear on neither side.
        b.finish(1, Duration::from_secs(600), true).unwrap();
        let left = b
            .remaining()
            .expect("a measured chapter is enough to predict");
        assert!(
            left.as_secs() > 450 && left.as_secs() < 750,
            "expected roughly 10 minutes for the one remaining chapter, got {}",
            fmt_dur(left)
        );

        // Finishing a restored row keeps the tag: the run does walk it.
        b.finish(0, Duration::from_millis(2), true).unwrap();
        assert!(b.rows[0].restored);
    }
