...
/// `translatus mcp` with no subcommand must keep launching the server: that
/// exact string is what every registered client has in its config, so adding
/// subcommands must never change what a bare `mcp` does.
#[test]
fn bare_mcp_still_launches_the_server() {
    let out = std::process::Command::new(env!("CARGO_BIN_EXE_translatus"))
        .arg("mcp")
        .arg("--help")
        .output()
        .expect("run");
    ...
    assert!(
        ...
        "install/uninstall must be discoverable from `mcp --help`: {help}"
    );
}
