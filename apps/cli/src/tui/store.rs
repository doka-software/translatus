...
/// Marker for "we have already offered to register the MCP server".
///
/// A marker file rather than a settings field: this is a one-time CLI-side
/// event, not a preference the engine or a host app has any business reading,
/// and it must not become one more thing that a settings migration can break.
/// Its presence means "asked", never "answered yes" — declining is permanent
/// too, because asking twice is nagging.
fn mcp_offer_marker() -> Option<PathBuf> {
    config_dir().map(|d| d.join("mcp-offer-done"))
}

pub fn mcp_offer_pending() -> bool {
    match mcp_offer_marker() {
        Some(p) => !p.exists(),
        None => false,
    }
}

pub fn mark_mcp_offer_done() {
    if let Some(p) = mcp_offer_marker() {
