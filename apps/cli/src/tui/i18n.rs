...
//! terminology researched for the GUI surface (margin notes = 註解/注解/
...
fn t(key: &str) -> [&'static str; 5] {
    match key {
        ...
        // ── brand banner ────────────────────────────────────────────────
        // The product's own line, not a slogan to re-invent per locale: the
        // zh-Hant wording is the one the website ships.
        "brand.tagline" => ["From any book, to your book.", "從一本書，到一本你的書。", "从一本书，到一本你的书。", "一冊の本から、あなたの一冊へ。", "한 권의 책에서, 당신의 책으로."],
        // ── main menu ───────────────────────────────────────────────────
        ...
        "menu.annotate" => ["Annotate", "註解", "注解", "AI注釈", "AI 주석"],
        "menu.annotate.sub" => ["Add margin notes, keeping the original text", "保留原文，只加註解", "保留原文，只加注解", "原文はそのまま、AI注釈だけを追加", "원문은 그대로 두고 AI 주석만 추가"],
        ...
        // Resume: a book with a job cache beside it that stopped part-way.
        "list.resume" => ["{}/{} chapters done", "已完成 {}/{} 章", "已完成 {}/{} 章", "{}/{} 章 完了", "{}/{}장 완료"],
        "list.resume.nototal" => ["{} chapters done", "已完成 {} 章", "已完成 {} 章", "{} 章 完了", "{}장 완료"],
        ...
        "cfg.h.notes" => ["Margin notes", "註解", "注解", "AI注釈", "AI 주석"],
        "cfg.h.model" => ["Model", "模型", "模型", "モデル", "모델"],
        "cfg.h.output" => ["Output", "輸出", "输出", "出力", "출력"],
        "cfg.translate" => ["Translate", "翻譯", "翻译", "翻訳する", "번역"],
        "cfg.translate.help" => ["Off, with notes on, keeps the original text and only adds margin notes.", "關閉翻譯、開著註解＝保留原文、只加註解。", "关闭翻译、开着注解＝保留原文、只加注解。", "オフにして注釈のみオンにすると、原文のまま注釈だけ追加します。", "번역을 끄고 주석만 켜면 원문은 그대로 두고 주석만 추가합니다."],
        ...
        "cfg.notes" => ["Add notes", "加註解", "加注解", "AI注釈を付ける", "AI 주석 추가"],
        "cfg.notes.help" => ["Background notes written for you specifically, placed where you are likely to want them.", "專為你寫的背景註解，放在你最可能需要的地方。", "专为你写的背景注解，放在你最可能需要的地方。", "あなたのために書かれた背景注釈を、必要になりそうな箇所に置きます。", "나를 위해 쓰인 배경 주석을 필요할 만한 곳에 배치합니다."],
        ...
        "cfg.services.help" => ["What the notes should do for you. Picking is enough; the box below is optional precision.", "註解要為你做什麼。用勾的就夠；下面的欄位是選填的補充。", "注解要为你做什么。用勾的就够；下面的栏位是选填的补充。", "注釈に何をしてほしいか。選ぶだけで十分。下の欄は任意の補足です。", "주석이 무엇을 해 줄지 선택하세요. 선택만으로 충분하며 아래 칸은 선택 사항입니다."],
        ...
        "cfg.anchors.help" => ["Things you know well, comma-separated. Notes explain new ideas by bridging from these.", "你熟悉的領域，逗號分隔。註解會從這些地方搭橋解釋新概念。", "你熟悉的领域，逗号分隔。注解会从这些地方搭桥解释新概念。", "詳しい分野をカンマ区切りで。注釈はそこから橋を架けて新しい概念を説明します。", "잘 아는 분야를 쉼표로 구분해 적으세요. 주석이 여기서 다리를 놓아 새 개념을 설명합니다."],
        ...
        "cfg.voice.companion" => ["spoken (close)", "口語（親近）", "口语（亲近）", "口語（親しみ）", "구어 (친근)"],
        "cfg.voice.help" => ["study = quiet scholar in the margin · spoken = everyday speech, more short reactions.", "書齋＝頁邊安靜的學者 · 口語＝日常說話的口吻，短反應多。", "书斋＝页边安静的学者 · 口语＝日常说话的口吻，短反应多。", "書斎＝余白の静かな学者 · 口語＝日常の話し言葉、短い反応が多め。", "서재 = 여백의 조용한 학자 · 구어 = 일상적인 말투, 짧은 반응이 많음."],
        ...
        "cfg.density.help" => ["How often notes appear.", "註解出現的頻率。", "注解出现的频率。", "注釈の頻度です。", "주석이 나타나는 빈도입니다."],
        "cfg.notelang" => ["Notes language", "註解語言", "注解语言", "注釈の言語", "주석 언어"],
        ...
        "cfg.nothing.l1" => ["  Both services are switched off, so there is no run to make.", "  翻譯與註解都關著，沒有可以執行的工作。", "  翻译与注解都关着，没有可以执行的工作。", "  翻訳もAI注釈もオフのため、実行するものがありません。", "  번역과 AI 주석이 모두 꺼져 있어 실행할 작업이 없습니다."],
        "cfg.nothing.l2" => ["  Switch on Translate, Add notes, or both.", "  打開「翻譯」或「加註解」，或兩個都開。", "  打开「翻译」或「加注解」，或两个都开。", "  「翻訳する」か「AI注釈を付ける」をオンにしてください。", "  '번역' 또는 'AI 주석 추가'를 켜 주세요."],
        "cfg.who.title" => ["Notes need to know who they are for", "註解需要知道是寫給誰的", "注解需要知道是写给谁的", "AI注釈には読み手の手がかりが必要です", "AI 주석에는 독자에 대한 단서가 필요합니다"],
        "cfg.who.l1" => ["  Margin notes are written for a specific reader — that is the", "  註解是為特定讀者而寫的，這正是它的", "  注解是为特定读者而写的，这正是它的", "  AI注釈は特定の読み手のために書かれます。", "  AI 주석은 특정 독자를 위해 쓰입니다."],
        ...
        "cfg.who.l5" => ["  \"Why this book\", or switch notes off.", "  寫一句話，或把註解關掉。", "  写一句话，或把注解关掉。", "  「この本を読む理由」に一行書くか、注釈をオフに。", "  '이 책을 읽는 이유'에 한 줄 쓰거나 주석을 끄세요."],
        ...
        "mcp.offer.title" => ["Connect this to your AI agent?", "要接上你的 AI agent 嗎？", "要接上你的 AI agent 吗？", "AI エージェントに接続しますか？", "AI 에이전트에 연결할까요?"],
        "mcp.offer.found" => ["Found on this machine: {}", "在這台電腦上找到：{}", "在这台电脑上找到：{}", "このマシンで見つかりました: {}", "이 컴퓨터에서 발견: {}"],
        "mcp.offer.what" => ["Registering lets it estimate, translate and annotate books for you by name.", "註冊之後，你可以直接用講的請它估價、翻譯、寫註解。", "注册之后，你可以直接用讲的请它估价、翻译、写注解。", "登録すると、見積もり・翻訳・注釈を会話で頼めるようになります。", "등록하면 견적·번역·주석을 말로 요청할 수 있습니다."],
        "mcp.offer.undo" => ["Undo any time: translatus mcp uninstall", "隨時可以取消：translatus mcp uninstall", "随时可以取消：translatus mcp uninstall", "いつでも解除できます: translatus mcp uninstall", "언제든 해제 가능: translatus mcp uninstall"],
        "mcp.offer.gate" => ["Register with the agents found above", "註冊到上面找到的 agent", "注册到上面找到的 agent", "上記のエージェントに登録", "위에서 찾은 에이전트에 등록"],
        "mcp.res.added" => ["registered as `translatus`", "已註冊為 `translatus`", "已注册为 `translatus`", "`translatus` として登録しました", "`translatus`(으)로 등록했습니다"],
        "mcp.res.removed" => ["removed", "已移除", "已移除", "削除しました", "삭제했습니다"],
        "mcp.res.restart" => ["Restart the agent to pick up: estimate_book, translate_book, annotate_book.", "重開 agent 後就會出現三個工具：estimate_book、translate_book、annotate_book。", "重开 agent 后就会出现三个工具：estimate_book、translate_book、annotate_book。", "エージェントを再起動すると estimate_book / translate_book / annotate_book が使えます。", "에이전트를 다시 시작하면 estimate_book, translate_book, annotate_book을 쓸 수 있습니다."],
        "mcp.res.none" => ["No agent client found on PATH (looked for: claude, codex).", "PATH 上找不到 agent（找過：claude、codex）。", "PATH 上找不到 agent（找过：claude、codex）。", "PATH にエージェントが見つかりません（claude, codex を探索）。", "PATH에서 에이전트를 찾지 못했습니다 (claude, codex 확인)."],
        "mcp.res.manual" => ["Install one, or register by hand:", "裝一個，或手動註冊：", "装一个，或手动注册：", "いずれかを導入するか、手動で登録してください:", "하나를 설치하거나 직접 등록하세요:"],
        "mcp.list.sep" => [", ", "、", "、", "、", ", "],
        "mcp.offer.done" => ["Agent setup", "Agent 設定", "Agent 设置", "エージェント設定", "에이전트 설정"],
        "run.left" => ["~{} left", "約剩 {}", "约剩 {}", "残り約 {}", "약 {} 남음"],
        "run.prescan" => ["reading the whole book to lock terminology…", "正在通讀全書、鎖定術語……", "正在通读全书、锁定术语……", "用語を固定するため全体を通読中…", "용어를 고정하기 위해 책 전체를 읽는 중…"],
        "run.noteplan" => ["mapping the book to decide where notes belong…", "正在盤點全書、決定註解落點……", "正在盘点全书、决定注解落点……", "注をどこに置くか全体を確認中…", "주석을 어디에 둘지 책 전체를 살피는 중…"],
        "run.chapters" => ["chapters", "章", "章", "章", "챕터"],
        // Restored rows carry a tag, so a chapter that lands instantly reads as
        // "already paid for" rather than "suspiciously fast".
        "run.cached" => ["cached", "快取", "缓存", "キャッシュ", "캐시"],
        "run.fromcache" => ["  {} restored from cache — not billed again", "  {}從快取還原，不重複計費", "  {}从缓存还原，不重复计费", "  {}をキャッシュから復元（再課金なし）", "  {}을 캐시에서 복원 (재과금 없음)"],
        "run.notesfromcache" => ["  {} notes restored from cache", "  {} 則註解從快取還原", "  {} 条注解从缓存还原", "  {} 件の注釈をキャッシュから復元", "  주석 {}개를 캐시에서 복원"],
        ...
        "run.stop.expert" => ["  ^C  stop — finished chapters are kept; the one in progress restarts", "  ^C 停止：已完成的章節會保留，正在跑的那章會重來", "  ^C 停止：已完成的章节会保留，正在跑的那章会重来", "  ^C 停止: 完了した章は保持、進行中の章はやり直し", "  ^C 중지: 완료된 장은 유지, 진행 중인 장은 다시 시작"],
        "run.resume.expert" => ["re-run the same command — finished chapters are free; the interrupted chapter starts over", "重跑同一個指令：已完成的章節免費，被中斷的那章會重來", "重跑同一个指令：已完成的章节免费，被中断的那章会重来", "同じコマンドを再実行: 完了章は無料、中断した章はやり直し", "같은 명령을 다시 실행: 완료된 장은 무료, 중단된 장은 처음부터"],
        ...
        "set.services.help" => ["Default note angles for new books.", "新書的預設註解角度。", "新书的默认注解角度。", "新しい本の注釈の既定の観点。", "새 책의 기본 주석 관점."],
        "set.notedensity" => ["Note density", "註解密度", "注解密度", "AI注釈の密度", "AI 주석 밀도"],
        ...
        "leg.jump" => ["jump", "直接選", "直接选", "番号で選択", "번호로 선택"],
        "leg.quit" => ["quit", "離開", "退出", "終了", "종료"],
        "leg.newline" => ["new line", "換行", "换行", "改行", "줄 바꿈"],
        "multi.selected" => ["{} selected", "已選 {}", "已选 {}", "{} 件選択", "{}개 선택"],
        // ── confirm gate ────────────────────────────────────────────────
        "go.cost.title" => ["What this would cost", "這樣會花多少", "这样会花多少", "かかる費用", "예상 비용"],
        "go.est" => ["  Cost  at least ${}", "  費用  至少 ${}", "  费用  至少 ${}", "  費用  最低 ${}", "  비용  최소 ${}"],
        "go.work" => ["  Work  at least {} tokens sent to the model", "  工作量  至少送出 {} tokens 給模型", "  工作量  至少送出 {} tokens 给模型", "  作業量  モデルへ最低 {} トークン送信", "  작업량  모델에 최소 {} 토큰 전송"],
        "go.floor" => ["  A floor, not a quote: prompts, retries and repairs are on top, and long books drift further.", "  這是下限不是報價：prompt、重試與修補都在這之上，書越長偏差越大。", "  这是下限不是报价：prompt、重试与修补都在这之上，书越长偏差越大。", "  下限であり見積ではありません: プロンプト・再試行・修復は別途、長い本ほど乖離します。", "  견적이 아니라 하한선입니다: 프롬프트·재시도·보정은 별도이며 긴 책일수록 커집니다."],
        "go.skipped" => ["  Not sent to any model: {} (publisher apparatus)", "  不會送給模型：{}（版權頁等出版社資訊）", "  不会送给模型：{}（版权页等出版社信息）", "  モデルに送りません: {}（奥付など）", "  모델에 보내지 않음: {}（판권지 등）"],
        ...
        "gate.work" => ["at least {} tokens", "至少 {} tokens", "至少 {} tokens", "最低 {} トークン", "최소 {} 토큰"],
        "gate.cost" => ["at least ${}", "至少 ${}", "至少 ${}", "最低 ${}", "최소 ${}"],
        "go.resume" => ["Resuming: {} already finished and not billed again — this run pays for the remaining {}.", "續跑：已完成 {}，不會重複計費；這次只算剩下的 {}。", "续跑：已完成 {}，不会重复计费；这次只算剩下的 {}。", "再開：完了済みの{}は再課金されません。今回の対象は残りの{}です。", "이어서 실행: 이미 완료한 {}은 다시 과금되지 않으며, 이번에는 남은 {}만 계산합니다."],
        "go.resume.keys" => ["Changing the model or the depth voids all of it and the book starts over.", "換模型或換深度會讓這些全部作廢，整本重跑。", "换模型或换深度会让这些全部作废，整本重跑。", "モデルか翻訳の深さを変えると、これらはすべて無効になり最初からやり直しになります。", "모델이나 번역 깊이를 바꾸면 모두 무효가 되어 처음부터 다시 실행됩니다."],
        "go.resume.stale" => ["This book has a saved job, but from a different model, depth or note setting — none of it can be reused.", "這本書有存檔，但當初用的是別的模型、深度或註解設定：這次無法沿用，整本重跑。", "这本书有存档，但当初用的是别的模型、深度或注解设置：这次无法沿用，整本重跑。", "この本には保存済みジョブがありますが、モデル・深さ・注釈設定が異なるため再利用できません。", "이 책에는 저장된 작업이 있지만 모델·깊이·주석 설정이 달라 재사용할 수 없습니다."],
        "go.resume.newjob" => ["You changed \"Save to\", so this run opens a new job cache: the {} finished under the default name are not reused.", "你改了「存到」，這次會開一份新的存檔：原本用預設檔名完成的 {} 不會沿用。", "你改了「存到」，这次会开一份新的存档：原本用默认档名完成的 {} 不会沿用。", "「保存先」を変更したため、新しいジョブとして開始します。既定の名前で完了した{}は再利用されません。", "'저장 위치'를 바꿔서 새 작업으로 시작합니다. 기본 이름으로 완료한 {}은 재사용되지 않습니다."],
        "go.free" => ["free", "免費", "免费", "無料", "무료"],
        "go.mock" => ["mock provider — nothing is sent anywhere and nothing is billed", "mock 供應商：不會送出任何東西，也不會計費", "mock 供应商：不会送出任何东西，也不会计费", "mock プロバイダ: どこにも送信されず、課金もありません", "mock 제공자: 아무것도 전송되지 않으며 과금도 없습니다"],
        "go.unpriced" => ["unpriced model", "無價目模型", "无价目模型", "価格未登録モデル", "가격 미등록 모델"],
        "gate.both" => ["Translate {} into {to} and add margin notes", "把 {} 翻成 {to} 並加上註解", "把 {} 翻成 {to} 并加上注解", "{} を {to} に翻訳し、AI注釈を追加", "{} 를 {to} 로 번역하고 AI 주석 추가"],
        "gate.translate" => ["Translate {} into {to}", "把 {} 翻成 {to}", "把 {} 翻成 {to}", "{} を {to} に翻訳", "{} 를 {to} 로 번역"],
        "gate.annotate" => ["Add margin notes to {}, keeping the original text", "為 {} 加註解，保留原文", "为 {} 加注解，保留原文", "{} にAI注釈を追加（原文はそのまま）", "{} 에 AI 주석 추가 (원문 유지)"],
...
mod tests {
    ...
    /// 2026-09-01: the outward name for annotations is 註解 (zh-Hant) / 注解
    /// (zh-Hans). The words it replaced are refused by the marketing copy lint,
    /// and this screen is what the website screenshots — so a row must never
    /// quietly get one back.
    ///
    /// The retired terms are written as escapes so that this test does not put
    /// them into the very file it scans.
    #[test]
    fn the_retired_annotation_terms_stay_out_of_every_row() {
        let src = include_str!("i18n.rs");
        for retired in ["\u{7709}\u{6279}", "\u{6279}\u{6ce8}", "\u{6279}\u{8a3b}"] {
            assert!(
                !src.contains(retired),
                "retired product term `{retired}` is back in i18n.rs"
            );
        }
        // …and the replacement really is in the rows, not just absent.
        assert_eq!(t("menu.annotate")[1], "註解");
        assert_eq!(t("menu.annotate")[2], "注解");
        assert_eq!(t("cfg.notelang")[1], "註解語言");
        assert_eq!(t("set.notedensity")[1], "註解密度");
    }

    /// 2026-09-01: the second voice register is named for the REGISTER it
    /// writes in (口語 / 구어 / spoken), not for a companion sitting beside the
    /// reader. The persona reading is refused by the marketing copy lint, and
    /// the notes themselves are forbidden to address the reader at all, so the
    /// old label promised something the engine will not do.
    ///
    /// The identifier is untouched: `--note-voice companion` and the
    /// `NoteVoice::Companion` variant (which enters the annotation signature)
    /// mean exactly what they meant before. Only the label moved.
    ///
    /// Retired terms are written as escapes so this test does not put them back
    /// into the file it scans.
    #[test]
    fn the_retired_voice_labels_stay_out_of_every_row() {
        let src = include_str!("i18n.rs");
        for retired in [
            "\u{966a}\u{8b80}",          // zh-Hant
            "\u{966a}\u{8bfb}",          // zh-Hans
            "\u{4f34}\u{8aad}",          // ja
            "\u{b3d9}\u{bc18}",          // ko
            "companion (\u{66}riendly)", // en
        ] {
            assert!(
                !src.contains(retired),
                "retired voice label `{retired}` is back in i18n.rs"
            );
        }
        // …and the replacement really is in the row.
        assert_eq!(t("cfg.voice.companion")[0], "spoken (close)");
        assert_eq!(t("cfg.voice.companion")[1], "口語（親近）");
        assert_eq!(t("cfg.voice.companion")[2], "口语（亲近）");
        assert_eq!(t("cfg.voice.companion")[3], "口語（親しみ）");
        assert_eq!(t("cfg.voice.companion")[4], "구어 (친근)");
        // The register it contrasts with is unchanged.
        assert_eq!(t("cfg.voice.study")[1], "書齋（克制）");
    }

    /// Chrome that used to be hard-coded English: the banner tagline and the
    /// main menu's own legend. A half-translated footer under a translated menu
    /// is the tell that a string never went through this table.
    #[test]
    fn banner_and_menu_chrome_are_translated_everywhere() {
        for key in ["brand.tagline", "leg.jump", "leg.quit", "leg.newline"] {
            let row = t(key);
            for (i, s) in row.iter().enumerate() {
                assert!(!s.is_empty(), "{key}[{i}] is empty");
            }
            assert_ne!(row[0], row[1], "{key} zh-Hant is still the English string");
            assert_ne!(row[0], row[3], "{key} ja is still the English string");
            assert_ne!(row[0], row[4], "{key} ko is still the English string");
        }
    }
