...
fn models_for(source: &str) -> Vec<String> {
    match source {
        "api key" => vec![
            "gpt-5.4-mini".into(),
            "gpt-5.4".into(),
...
fn default_output_path(input: &Path, to: &str, translate: bool) -> std::path::PathBuf {
    ...
    let name = if translate {
        let slug = crate::safe_filename_component(to);
        format!("{stem}.{slug}.{ext}")
    } else {
        format!("{stem}.annotated.{ext}")
    };
...
pub fn run(root: &Path) -> Result<Intent> {
    ...
    loop {
        ...
        // pre-flipped, not a second implementation.
...
fn config_screen(
    ...
    let mut refresh = |fields: &mut [Field]| {
        ...
        let valid = models_for(source_id_of_label(fields[F_SOURCE].current()));
...
fn test_connection_blocking(settings: &Settings) -> Vec<String> {
    ...
    out.push(format!("  {:<10}{}", tr("test.model"), cfg.model));
    out.push(format!(
        "  {:<10}{}",
        if provider_to_source(settings) == "subscription" {
            tr("set.token")
        } else {
            tr("test.keyrow")
        },
...
mod tests {
    ...
    fn every_offered_source_maps_to_a_provider_the_engine_accepts() {
        ...
        for id in SOURCE_IDS {
            ...
            assert!(!models_for(id).is_empty(), "source {id:?} offers no models");
        }
    }
