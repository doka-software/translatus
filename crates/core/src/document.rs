...
pub struct Chapter {
    ...
    /// Publisher apparatus (copyright page, imprint, 奥付) rather than the
    /// book. Set at parse time; the pipeline neither translates nor annotates
    /// it, and the writer emits it byte-identical.
    #[serde(default)]
    pub apparatus: bool,
}
...
impl Book {
    ...
    /// Tokens the run will actually send. Publisher apparatus is excluded
    /// because the pipeline never sends it.
    pub fn est_source_tokens(&self) -> usize {
        self.chapters
            .iter()
            .filter(|c| !c.apparatus)
            .flat_map(|c| c.segments.iter())
            .map(|s| s.est_tokens())
            .sum()
    }

    /// Tokens the run will send, ignoring chapters a previous run already
    /// finished. `skip` holds chapter indices restored from the job cache: a
    /// resume must be priced on what is left, not on the whole book.
    pub fn est_source_tokens_excluding(&self, skip: &[usize]) -> usize {
        self.chapters
            .iter()
            .enumerate()
            .filter(|(i, c)| !c.apparatus && !skip.contains(i))
            .flat_map(|(_, c)| c.segments.iter())
            .map(|s| s.est_tokens())
            .sum()
    }

    /// Chapters skipped as publisher apparatus, for reporting.
    pub fn apparatus_chapters(&self) -> Vec<&Chapter> {
        self.chapters.iter().filter(|c| c.apparatus).collect()
    }
}
...
mod tests {
    ...
    /// A resume is priced on what is left. Charging for chapters the cache will
    /// restore is not a safety margin, it is the wrong number — and publisher
    /// apparatus stays free either way, because it is never sent.
    #[test]
    fn a_resume_is_priced_on_the_chapters_still_to_run() {
        fn chapter(idx: usize, text: &str, apparatus: bool) -> Chapter {
            Chapter {
                spine_index: idx,
                href: format!("c{idx}.xhtml"),
                title: None,
                apparatus,
        ...
        let book = Book {
            ...
            title: None,
            chapters: vec![
                chapter(0, "copyright page", true),
                chapter(1, "the first chapter of the book", false),
                chapter(2, "the second chapter of the book", false),
            ],
        };
        let whole = book.est_source_tokens();
        assert_eq!(book.est_source_tokens_excluding(&[]), whole);
        // Apparatus was never in the number, so skipping it changes nothing.
        assert_eq!(book.est_source_tokens_excluding(&[0]), whole);
        // One finished chapter drops out; the other still has to be paid for.
        let left = book.est_source_tokens_excluding(&[0, 1]);
        assert!(left > 0 && left < whole, "{left} of {whole}");
        assert_eq!(book.est_source_tokens_excluding(&[0, 1, 2]), 0);
    }
