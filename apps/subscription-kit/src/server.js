...
import { openAiUsage, totalInputTokens } from "./providers/usage.js";
...
async function handleCompletion(body, apiKey, abortController, providers) {
  ...
  // `usage.input_tokens` is the full-rate remainder only; the log has to name
  // the real total or a cached run looks like a 10-token request.
  ...
  const cacheNote =
    usage.cache_read_input_tokens || usage.cache_creation_input_tokens
      ...
      : "";
  console.error(
    `[llm-sub-kit] requested=${model} reported=${reportedModel ?? "n/a"} ${apiKey ? "api-key" : "subscription"} ok ${Date.now() - t0}ms in=${inTotal}${cacheNote} out=${usage.output_tokens}`,
  );
  return {
    id: `lsk-${Date.now()}`,
