...
import { emptyUsage, normalizeClaudeUsage } from "./usage.js";
  ...
  try {
    ...
    for await (const m of q) {
      ...
      if (m.type === "result") {
        ...
        // Not `m.usage.input_tokens` alone: with prompt caching on (the SDK
        // caches every call) that field is only the uncached remainder — see
        // providers/usage.js.
        if (m.usage) usage = normalizeClaudeUsage(m.usage);
