...
pub fn write(
    ...
    let note_lang = note_lang.map(super::epub::lang_attr);
    let mut paras = doc.paragraphs.clone();
    if let Some(chapter) = book.chapters.first().filter(|c| !c.apparatus) {
        for seg in &chapter.segments {
            ...
            if let Some(n) = seg.note.as_ref().filter(|n| !n.is_skip()) {
                if let Some(slot) = paras.get_mut(seg.block_index) {
                    // Notes are prose in the reader's language, so they get
                    // the same typography normalisation as the translation
                    // (interpunct + CJK punctuation) under their OWN language,
                    // exactly as the EPUB writer does.
                    let text = crate::format::typography::normalize(
                        &n.text,
                        note_lang.as_deref().unwrap_or(&lang),
                    );
                    *slot = match n.pos {
                        crate::document::NotePos::Before => {
                            format!("{} {}\n{}", crate::format::dom::NOTE_PREFIX, text, slot)
                        }
                        crate::document::NotePos::After => {
                            format!("{}\n{} {}", slot, crate::format::dom::NOTE_PREFIX, text)
                        }
                    };
                }
            }
...
mod tests {
    ...
    fn write_restores_glossary_sentinels() {
        ...
        write(&doc, &book, &out, OutputMode::Replace, "English", None).unwrap();
        let got = std::fs::read_to_string(&out).unwrap();
        let _ = std::fs::remove_file(&out);
        assert_eq!(got.trim(), "亞哈船長");
    }

    /// The TXT writer used to emit note text raw, so a note carrying the
    /// model's ASCII commas reached the reader unrepaired even though the EPUB
    /// writer fixed the very same note. Notes normalise under their OWN
    /// language, not the translation target.
    #[test]
    fn write_normalises_note_typography_under_the_note_language() {
        let out = std::env::temp_dir().join("et_txt_note_typography_test.txt");
        let mut seg = Segment::new(0, "Ahab".into(), Default::default());
        seg.note = Some(crate::document::Note::new(
            crate::document::NotePos::After,
            "帳篷不是租來的成品,是工人現場搭建的,靠木樁撐起。",
        ));
        let chapter = Chapter {
            spine_index: 0,
            href: "c".into(),
        ...
        let _ = std::fs::remove_file(&out);
        assert!(
            got.contains("帳篷不是租來的成品，是工人現場搭建的，靠木樁撐起。"),
            "note must be normalised: {got}"
        );
        assert!(!got.contains(','), "no ASCII comma may survive: {got}");
    }
}
