...
fn apply_configured_model_source(
    ...
    if provider.is_empty() || provider == "mock" {
        return Err(ToolError::InvalidParams(
            "no model source is configured, and this server will not silently \
             dry-run a book. Either pass provider/model (and base_url for a \
             local endpoint), or run `translatus` once and set the model source \
             under Settings — after that this tool uses it automatically."
                .into(),
        ));
    }
