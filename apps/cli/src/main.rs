...
enum Command {
    ...
    Mcp(McpArgs),
    /// Health check: binary/install self-consistency, subscription sidecar,
    /// default sidecar port, per-client MCP registration, settings file.
    /// Exit code 1 only on a hard failure.
    Doctor,
}
...
enum McpCommand {
    ...
    Install {
        /// Register with one specific client only (claude | codex).
        #[arg(long)]
        client: Option<String>,
    },
    /// Undo `install`. Without `--client`, removes only the registrations that
    /// `translatus mcp install` itself created — never one the user (or
    /// another tool) added. Pass `--client <name>` to remove a specific
    /// client's registration regardless of who created it.
    Uninstall {
        /// Unregister from one specific client (claude | codex), even if the
        /// registration was not created by `translatus mcp install`.
        #[arg(long)]
        client: Option<String>,
    },
}
...
struct TranslateArgs {
    ...
    /// bilingual | replace.
    #[arg(long, default_value = "bilingual")]
    mode: String,
    ...
    /// Note voice register: study (書齋, default) | companion (口語).
    #[arg(long)]
...
struct EstimateArgs {
    ...
    density: String,
    /// Accepted and ignored, so a translate command line can be replayed as an
    /// estimate by swapping the verb. An estimate parses and counts locally —
    /// it never contacts a provider — so none of these change the number.
    /// (Real-world failure: an agent copied its translate flags onto estimate
    /// and hit a hard `unexpected argument` error.)
    #[command(flatten)]
    ignored: IgnoredTranslateFlags,
}

/// Translate-only flags that `estimate` accepts for command-line symmetry and
/// deliberately ignores. Hidden from `estimate --help` to keep it honest about
/// what actually affects the estimate.
#[derive(Parser)]
struct IgnoredTranslateFlags {
    #[arg(long, hide = true)]
...
impl IgnoredTranslateFlags {
    /// The flags the caller actually passed, for one honest stderr notice.
    fn named(&self) -> Vec<&'static str> {
        ...
        if self.note_presets.is_some() {
            v.push("--note-presets");
        }
        if self.note_level.is_some() {
            v.push("--note-level");
        }
        if self.note_voice.is_some() {
            v.push("--note-voice");
        }
        if self.note_anchors.is_some() {
            v.push("--note-anchors");
        }
        if self.note_lang.is_some() {
            v.push("--note-lang");
        }
        if self.note_profile.is_some() {
            v.push("--note-profile");
        }
        v
    }
...
async fn main() -> Result<()> {
    ...
    match cmd {
        Command::Doctor => std::process::exit(doctor::run(cli.json)),
        ...
        Command::Mcp(a) => match a.cmd {
            ...
            Some(McpCommand::Install { client }) => {
                for l in mcp::register(true, client.as_deref())? {
                    println!("{l}");
                }
                Ok(())
            }
            Some(McpCommand::Uninstall { client }) => {
                for l in mcp::register(false, client.as_deref())? {
                    println!("{l}");
                }
                Ok(())
            }
...
pub(crate) fn default_output(input: &Path, lang: &str) -> PathBuf {
...
/// Where an annotate-only run writes when `--output` is not given.
pub(crate) fn default_annotate_output(input: &Path) -> PathBuf {
    let ext = input.extension().and_then(|e| e.to_str()).unwrap_or("out");
    let stem = input.file_stem().and_then(|s| s.to_str()).unwrap_or("book");
    input.with_file_name(format!("{stem}.annotated.{ext}"))
}

/// The job cache a run writes beside its output. Resume works by two runs
/// landing on the same job file, so this derivation is the thing the
/// interactive session's resume probe has to agree with — hence one
/// definition, called from both, rather than a copy per caller.
pub(crate) fn job_path_for(output: &Path) -> PathBuf {
    output.with_extension("etjob")
}
...
/// The full engine config a `translate` run uses, annotations included.
///
/// Sole definition of everything `cache_signature()` and
/// `annotation_signature()` hash. The interactive session previews a resume by
/// asking this for the very config the run will use; a second, hand-rolled
/// copy in the TUI would eventually drift and let the screen promise "8
/// chapters already done" for a job the run then re-translates in full.
pub(crate) fn translate_config(a: &TranslateArgs) -> Result<TranslateConfig> {
    let mut cfg = TranslateConfig::new(&a.to);
    cfg.level = a.level.parse().map_err(|e| anyhow!("{e}"))?;
    cfg.output_mode = a.mode.parse().map_err(|e| anyhow!("{e}"))?;
    cfg.provider = a.provider.parse().map_err(|e| anyhow!("{e}"))?;
...
/// An annotate-only run's config before annotations. Split out because
/// `--cache-only` deliberately returns before the annotation config is built
/// (it reads the signature the notes were written under from the job instead),
/// and building it early would turn a bad `--note-profile` into a failure on a
/// command that never reads it.
fn annotate_base_config(a: &AnnotateArgs) -> Result<TranslateConfig> {
...
/// The annotation settings an annotate-only run uses.
fn annotate_annotations(a: &AnnotateArgs) -> Result<AnnotationConfig> {
...
/// The full engine config an `annotate` run uses. The resume preview's
/// counterpart to [`translate_config`].
pub(crate) fn annotate_config(a: &AnnotateArgs) -> Result<TranslateConfig> {
    let mut cfg = annotate_base_config(a)?;
    cfg.annotations = Some(annotate_annotations(a)?);
    Ok(cfg)
}
...
fn estimate_numbers(
    ...
    estimate_numbers_from_tokens(
        book.est_source_tokens() as u64,
        model,
        level,
        annotate_density,
    )
...
/// The same arithmetic over a source-token count that is not a whole book.
/// A resume prices only the chapters still to do, and cloning a book minus its
/// finished chapters just to call the function above would double peak memory
/// on exactly the long books where resume matters.
fn estimate_numbers_from_tokens(
    source: u64,
    model: &str,
    level: Level,
    annotate_density: Option<Density>,
...
fn run_estimate(a: EstimateArgs, json_out: bool) -> Result<()> {
    let ignored = a.ignored.named();
    if !ignored.is_empty() {
        eprintln!(
            "note: {} accepted and ignored — an estimate parses and counts locally, \
...
/// One header line per `--json` run, so several runs appended to the same log
/// (interrupt → resume is the documented workflow) stay separable afterwards.
fn emit_run_start(json_out: bool) {
    if !json_out {
        return;
    }
    ...
    println!(
        "{}",
        json!({ "event": "run_start", "version": env!("CARGO_PKG_VERSION"), "ts": ts })
    );
}

async fn run_translate(mut a: TranslateArgs, json_out: bool, ui: Ui) -> Result<RunOutcome> {
    emit_run_start(json_out);
    let level: Level = a.level.parse().map_err(|e| anyhow!("{e}"))?;
    let mode: OutputMode = a.mode.parse().map_err(|e| anyhow!("{e}"))?;
    // `subscription` is a CLI-level alias, not an engine provider: it means
    // "start the sidecar for me, then talk to it like any OpenAI-compatible
    // endpoint". Keeping it out of the engine keeps the cache signature (which
    // hashes the provider) stable, so switching between a managed sidecar and a
    // hand-started one never re-bills a book.
    // Held for the whole run: dropping it shuts the sidecar down.
    let _managed = if a.provider == "subscription" {
        let m = sidecar::Managed::start(json_out)?;
        a.base_url = Some(m.base_url.clone());
        a.provider = "openai".into();
        // `--model` still holds clap's `mock` default unless the caller chose
        // one, and the sidecar does not serve a model called mock.
        if a.model == "mock" {
            a.model = sidecar::DEFAULT_MODEL.into();
        }
        Some(m)
    } else {
        None
    ...
    let job_path = a.job.clone().unwrap_or_else(|| job_path_for(&output));
    ensure_distinct_run_paths(&a.input, &output, &job_path)?;

    let cfg = translate_config(&a)?;
    let sig = cfg.cache_signature();
    let note_lang = cfg.annotations.as_ref().and_then(|an| an.lang.clone());

    // Resolve API key: a managed sidecar's own token > OS keychain > environment.
    // The managed token never goes near the keychain, which is the point: an
    // ad-hoc-signed binary cannot read its own keychain items without a dialog,
    // and there is nobody to click it in an agent session.
    let api_key = _managed
        .as_ref()
        .map(|m: &sidecar::Managed| m.token.clone())
        .or_else(|| resolve_api_key(provider_kind));

    let (mut book, doc) = format::extract(&a.input).with_context(|| "failed to parse input")?;
    let store = job::JobStore::open(&job_path)?;

    // Which chapters this run will restore rather than buy. Read BEFORE the run
    // stamps its own signature below: what matters is what the PREVIOUS run
    // left and whether these settings can still use it. Both caches have to
    // match — a chapter whose translation is cached but whose notes are about
    // to be rewritten is not "already done".
    let resumable_chapters: Vec<usize> = {
        let notes_reusable = match (cfg.annotation_signature(), store.get_meta("anno_sig")) {
            (None, _) => true, // notes are off; only the translation matters
            (Some(want), Ok(Some(prev))) => want == prev,
            _ => false,
        };
        match store.config_sig() {
            Ok(Some(prev)) if notes_reusable && prev == sig => {
                store.done_chapters().unwrap_or_default()
            }
            _ => Vec::new(),
        }
    };
    ...
    if let Some(b) = board.as_mut() {
        ...
        for idx in &resumable_chapters {
            b.mark_restored(*idx);
        }
        // Open the spinner on the first chapter that still has work, not on a
        // restored chapter 1 — the run passes through those in milliseconds.
        let first_todo = (0..total_chapters)
            .find(|i| !resumable_chapters.contains(i))
            .unwrap_or(0);
        let _ = b.start(first_todo);
    }
        ...
        match p.phase {
            ...
            translate::Phase::Translating => {
                if json_out {
                    ...
                    eprintln!(
                        "  [{}/{}] {} — skipped (publisher apparatus, sent to no model)",
                        p.chapter_index + 1,
                        p.total_chapters,
                        p.href
                    );
    ...
    // Cost is priced off `billable_tokens_in`, not `tokens_in`: prompt-cache
    // reads bill at 0.1x and writes at 1.25x/2x, so pricing the raw total at the
    // base input rate over-charges a cached run as badly as the old
    // cache-blind count under-charged it.
    let cost =
        et_core::estimate_cost_usd(&cfg.model, summary.billable_tokens_in, summary.tokens_out);
    let mut result = json!({
        "event": "done",
        ...
        // Input re-expressed in base-input-price units; equals tokens_in when
        // the provider used no prompt cache. `est_cost_usd` is derived from it.
        "billable_tokens_in": summary.billable_tokens_in,
        "est_cost_usd": cost,
    ...
    if let Some(b) = board.as_ref() {
        ...
        // The resume payoff, in the interactive session too. This number only
        // ever reached the flag-driven wrap-up below, so the one surface that
        // opens on a half-finished book never showed what it saved.
        if summary.restored_from_cache > 0 {
            println!(
                "{}",
                tui::i18n::tr1(
                    "run.fromcache",
                    &tui::i18n::trn("n.segment", summary.restored_from_cache)
                )
            );
        }
        if a.annotate && summary.notes_restored_from_cache > 0 {
            println!(
                "{}",
                tui::i18n::tr1(
                    "run.notesfromcache",
                    &summary.notes_restored_from_cache.to_string()
                )
            );
        }
        println!(
...
async fn run_annotate(mut a: AnnotateArgs, json_out: bool) -> Result<RunOutcome> {
    emit_run_start(json_out);
    let _managed = if a.provider == "subscription" {
        let m = sidecar::Managed::start(json_out)?;
        a.base_url = Some(m.base_url.clone());
    ...
    let output = a
        .output
        .clone()
        .unwrap_or_else(|| default_annotate_output(&a.input));
    let job_path = a.job.clone().unwrap_or_else(|| job_path_for(&output));
    ensure_distinct_run_paths(&a.input, &output, &job_path)?;
    ...
    if json_out {
        let mut result = json!({
            ...
            "tokens_in": summary.tokens_in,
            "tokens_out": summary.tokens_out,
            // Input re-expressed in base-input-price units; equals tokens_in
            // when the provider used no prompt cache. `est_cost_usd` uses it.
            "billable_tokens_in": summary.billable_tokens_in,
            "est_cost_usd": cost,
        });
        if let Some(e) = &summary.sample_error {
