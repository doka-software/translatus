...
import { normalizeClaudeUsage, normalizeCodexUsage, openAiUsage, totalInputTokens } from "../src/providers/usage.js";
...
// --- token accounting (offline; no LLM call) ---
// Regression guard for the cost bug: with prompt caching on, Anthropic bills the
// prefix under the cache fields and leaves only the remainder in `input_tokens`,
// so reading that field alone reported real 16k-token requests as ~10 tokens.
{
  // Shape captured from a real claude-haiku-4-5 call through this sidecar.
  const claudeWrite = normalizeClaudeUsage({
    input_tokens: 10,
    cache_creation_input_tokens: 16173,
    cache_read_input_tokens: 0,
    cache_creation: { ephemeral_1h_input_tokens: 16173, ephemeral_5m_input_tokens: 0 },
    output_tokens: 455,
  });
  check("claude: cache-write tokens counted as input", totalInputTokens(claudeWrite) === 16183, JSON.stringify(claudeWrite));
  check("claude: write TTL split preserved", claudeWrite.cache_creation_1h_input_tokens === 16173 && claudeWrite.cache_creation_5m_input_tokens === 0);

  // Same prompt, second call: the prefix is now a cache read.
  const claudeRead = normalizeClaudeUsage({
    input_tokens: 10,
    cache_creation_input_tokens: 0,
    cache_read_input_tokens: 16173,
    cache_creation: { ephemeral_1h_input_tokens: 0, ephemeral_5m_input_tokens: 0 },
    output_tokens: 466,
  });
  check("claude: cache-read tokens counted as input", totalInputTokens(claudeRead) === 16183);
  check("claude: cache read stays its own bucket (0.1x, not base rate)", claudeRead.cache_read_input_tokens === 16173 && claudeRead.input_tokens === 10);

  // Missing TTL split -> the unexplained remainder is treated as the expensive
  // 1h write, so the estimate stays a ceiling instead of under-quoting.
  const noSplit = normalizeClaudeUsage({ input_tokens: 5, cache_creation_input_tokens: 900, cache_read_input_tokens: 0, output_tokens: 1 });
  check("claude: unsplit cache writes attributed to the 1h bucket", noSplit.cache_creation_1h_input_tokens === 900 && noSplit.cache_creation_5m_input_tokens === 0);
  check("claude: absent usage object is zeroed, not NaN", totalInputTokens(normalizeClaudeUsage(undefined)) === 0);

  // Codex is the opposite convention: input_tokens already INCLUDES the cached
  // subset, so the total must not double-count.
  const codex = normalizeCodexUsage({ input_tokens: 12000, cached_input_tokens: 9000, output_tokens: 300, reasoning_output_tokens: 100 });
  check("codex: total input is not double-counted", totalInputTokens(codex) === 12000, JSON.stringify(codex));
  check("codex: cached subset split out for pricing", codex.cache_read_input_tokens === 9000 && codex.input_tokens === 3000);
  check("codex: no cache-write charge", codex.cache_creation_input_tokens === 0);

  // Wire contract: prompt_tokens is the honest total; the discounted buckets are
  // broken out so a caller can weight them (read 0.1x, 5m write 1.25x, 1h 2x).
  const wire = openAiUsage(claudeWrite);
  check("wire: prompt_tokens is the true total", wire.prompt_tokens === 16183);
  check("wire: total_tokens = prompt + completion", wire.total_tokens === 16183 + 455);
  check("wire: OpenAI-standard cached_tokens present", wire.prompt_tokens_details.cached_tokens === 0);
  check(
    "wire: cache-write breakdown present and self-consistent",
    wire.prompt_tokens_details.cache_creation_tokens === 16173 &&
      wire.prompt_tokens_details.cache_creation_5m_tokens + wire.prompt_tokens_details.cache_creation_1h_tokens === 16173,
  );
  const wireRead = openAiUsage(claudeRead);
  check("wire: cache reads surface as cached_tokens", wireRead.prompt_tokens_details.cached_tokens === 16173 && wireRead.prompt_tokens === 16183);
}
