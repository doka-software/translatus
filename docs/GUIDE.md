...
- `--note-voice study|companion` — the default style register: `study`
  (書齋, restrained written prose, the historical default) or `companion`
  (口語, everyday spoken register, more short reaction notes). A custom
  `--note-style` overrides either.
...
passes), so they are much larger than the visible note text. `tokens_in` counts
every input token the run actually read, including the ones a provider served
from its prompt cache; `billable_tokens_in` is that same input re-weighted into
base-input-price units (cached tokens bill at a fraction of the full rate, cache
writes at a premium), and it is the number `est_cost_usd` is derived from. With
no prompt caching in play the two are identical. Two note counters
